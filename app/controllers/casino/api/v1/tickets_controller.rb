module CASino
  module API
    module V1
      class TicketsController < CASino::ApplicationController

        # POST /cas/v1/tickets
        def create
          CASino::Processors::API::LoginCredentialAcceptor.new(self).process(params, request.user_agent)
        end

        # POST /cas/v1/tickets/{TGT id}
        def update
          CASino::Processors::API::ServiceTicketProvider.new(self).process(params[:id], params, request.user_agent)
        end

        # DELETE /cas/v1/tickets/TGT-fdsjfsdfjkalfewrihfdhfaie
        def destroy
          CASino::Processors::API::Logout.new(self).process(params[:id], request.user_agent)
        end
